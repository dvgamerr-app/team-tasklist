          ...
          <div class="box-filter">
            <b-dropdown slot="button" variant="outline-primary" right size="sm">
              ...
              <b-dropdown-item
                ...
                >Waiting</b-dropdown-item
              >
              <b-dropdown-item
                :active="filter.status === 3"
                href="#"
                class="f-sm"
                @click.prevent="setStatus(3)"
                >Complaed</b-dropdown-item
              >
            </b-dropdown>
          </div>
        </div>
    ...
    <div class="row">
      <div v-if="items.length === 0" class="col-md-24 offset-md-6 box f-sm">
        <div class="todos-empty-content">
          ...
          <p>
            When todo assigned to you this will trigger a new item in your todo
            list, automatically.
          </p>
      ...
      <div class="col-sm-36 box f-sm">
        <div
          v-for="e in getFilterItems()"
          :key="e._id"
          class="box-row pb-2 border-bottom pl-2 pt-1"
        >
          <span class="icon">
...
export default {
  ...
  methods: {
    getFilterItems() {
      ...
      data = data.sort((a, b) =>
        !a.duedate || !b.duedate ? -1 : a.duedate > b.duedate ? 1 : -1
      )
      return data
    },
    getDuedate(e) {
      const diff = !e.duedate ? null : dayjs().diff(dayjs(e.duedate), 'day')
      return e.duedate
    ...
    async reloadTodoItems() {
      const { data } = await this.$axios.get(
        `/api/todo/list/${this.filter.status}`
      )
      this.items = data
    },
  },
