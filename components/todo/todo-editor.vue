...
export default {
  props: {
    ...
    toolbar: {
      type: String,
      default:
        'bold italic strikethrough heading | image link | numlist bullist code quote | preview fullscreen',
    },
    name: {
      type: String,
      default: 'editor',
  ...
  methods: {
    groupButtons() {
      const groups = []
      let btns = []
      for (const e of vm.toolbar
        .toLowerCase()
        .split(/(\s)/)
        .filter((w) => !vm.isEmpty(w))) {
        if (e === '|') {
          if (btns.length > 0) groups.push(btns)
          btns = []
        } else {
          const btn = vm.buttons[e]
          if (!btn) continue
          btns.push({
            id: `${vm.id}-${e}`,
            cmd: btn.cmd,
    ...
    isEmpty: (s) =>
      s === null || s === undefined ? true : /^[\s\xA0]*$/.test(s),
    isUrl: (s) =>
      this.isEmpty(s)
        ? false
        : s.match(
